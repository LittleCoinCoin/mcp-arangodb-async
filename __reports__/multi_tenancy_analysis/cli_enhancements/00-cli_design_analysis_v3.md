...
This document specifies the final CLI design for ArangoDB user and database management. The design has been approved through stakeholder review (v0→v1→v2→v3).
...
### 3.3 Example: `db add` with `--with-user`
...
### 3.4 Example: `db remove` with Side Effects
...
### 3.6 Example: `--yes` Flag (Skip Confirmation)
...
All mutating commands collect consequences and report them uniformly:

```python
# Pseudo-code for consistent result reporting
reporter = ResultReporter("db add", dry_run=args.dry_run)
reporter.add(ADDED, f"Database '{name}'")
if with_user:
    reporter.add(ADDED, f"User '{user}' (active: true)")
    reporter.add(GRANTED, f"Permission {perm}: {user} → {name}")

if args.dry_run:
    reporter.report()  # Shows [* - DRY-RUN] prefix
else:
    execute_operations()
    reporter.report()  # Shows [*] prefix
```
...
| Comprehensive feedback | ✅ | Section 3.4: Side effects reported |
| Unified format for dry-run/execute | ✅ | Sections 3.3-3.6: Same format with `- DRY-RUN` suffix |
...
**Report Version**: v3 (Final Design)
